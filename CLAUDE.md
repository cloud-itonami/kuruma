...
この repo は corpus である。**上流を散文でなく目録で持つ。**

- 正本は `data/sources.edn`（収集源 / 市場統計源 / 規格源、および
  **載せなかった URL とその理由**、**まだ出典を持たないもの**）。
- 検査は `nbb scripts/verify_sources.cljs` —— 全 URL を実際に取得し、
  `:expect-status` と食い違えば **exit 1**。目録が読めない・URL が 0 件・
  全件が network error のときは **exit 3**（「答えられなかった」を
  「問題が無かった」と同じ値で返さない）。
- **引けなかった URL は載せない。** 落とした事実は `:rejected` に残す。

⚠ `scripts/seed_vehicles.mjs` の車両諸元は "real-world specs" と名乗るが
**出典が未確定**（`:gaps` に記録）。台数を増やす前に出典を決めること。
